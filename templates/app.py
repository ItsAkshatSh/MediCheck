...
def delete_medicine(med_id):
    ...
    if med.user_id != current_user.id:
        ...
        return redirect('/dashboard')
    # undo
    session['undo_medicine'] = json.dumps({
        'name': med.name,
        'expiry_date': med.expiry_date.strftime('%Y-%m-%d'),
        'notes': med.notes,
        'image_filename': med.image_filename
    })
    db.session.delete(med)
    db.session.commit()
    flash('Medicine deleted. <a href="/undo-delete" class="toast-undo">Undo</a>', 'success')
    return redirect('/dashboard')

@app.route('/undo-delete')
@login_required
def undo_delete():
    data = session.pop('undo_medicine', None)
    if not data:
        flash('Nothing to undo.', 'danger')
        return redirect('/dashboard')
    try:
        med_data = json.loads(data)
        from datetime import datetime
        med = Medicine(
            user_id=current_user.id,
            name=med_data['name'],
            expiry_date=datetime.strptime(med_data['expiry_date'], '%Y-%m-%d').date(),
            notes=med_data.get('notes'),
            image_filename=med_data.get('image_filename')
        )
        db.session.add(med)
        db.session.commit()
        flash('Medicine restored.', 'success')
    except Exception:
        flash('Failed to restore medicine.', 'danger')
    return redirect('/dashboard')
